...
const NoteCreateModal = ({ open = false, handleClose = () => {}, setNotes = () => {} }) => {
 ...
 const userToken=Cookies.get('UserToken');
    ...
    if(response.data.success) {
      ...
      resetAndClose();}
   }catch(error) {if(error instanceof AxiosError&&error.response) {
    setStatus({severity:'error',msg:error.response.data.error});}
    else{setStatus({severity:'error',msg:error.message});}}
  };
const validateForm=()=>{
  const error={};
  if(!newNote.title) 
  ...
  setError(error);if(Object.keys(error).length) return false;
  return true;}
  const resetAndClose = () => {
    setTimeout(() => {
      setNewNote({
